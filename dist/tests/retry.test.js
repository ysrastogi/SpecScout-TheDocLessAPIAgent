...
const retry_1 = require("../sdk/retry");
describe('RetryHandler', () => {
    let retryHandler;
    beforeEach(() => {
        retryHandler = new retry_1.RetryHandler({
            maxRetries: 3,
            baseDelay: 100,
            maxDelay: 5000,
            jitterFactor: 0.1,
    ...
    describe('parseRetryAfter', () => {
        it('should parse numeric retry-after values', () => {
            expect(retry_1.RetryHandler.parseRetryAfter('60')).toBe(60);
            expect(retry_1.RetryHandler.parseRetryAfter('0')).toBe(0);
            expect(retry_1.RetryHandler.parseRetryAfter('3600')).toBe(3600);
        });
        it('should parse HTTP date retry-after values', () => {
            const future = new Date(Date.now() + 60000); // 1 minute from now
            const retryAfter = retry_1.RetryHandler.parseRetryAfter(future.toUTCString());
            expect(retryAfter).toBeGreaterThan(50); // Should be close to 60 seconds
            expect(retryAfter).toBeLessThan(70);
        });
        it('should return undefined for invalid values', () => {
            expect(retry_1.RetryHandler.parseRetryAfter('')).toBeUndefined();
            expect(retry_1.RetryHandler.parseRetryAfter('invalid')).toBeUndefined();
        });
    });
    describe('createRetryableError', () => {
        it('should create retryable error with all properties', () => {
            const error = retry_1.RetryHandler.createRetryableError('Rate limit exceeded', 429, 60, true);
        ...
        it('should default isRetryable to true', () => {
            const error = retry_1.RetryHandler.createRetryableError('Server error', 500);
            expect(error.isRetryable).toBe(true);
        });
    });
